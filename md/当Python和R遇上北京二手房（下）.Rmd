...
# Boxplot 均值点
bmean <- stat_summary(fun.y="mean", geom="point", shape=19, size=2, color = "red")
```
...
p <- ggplot(houses, aes(area, num))
p + mtm + geom_boxplot() + bmean +
    coord_cartesian(ylim = c(0, 1200)) +
    ggtitle("北京二手房各区总价") + xlab("") + ylab("总价（万元）")
...
num_cg_count = ddply(houses, .(area, num_cg), summarise, count = length(num_cg))
num_cg_ratio = ddply(num_cg_count, .(area), transform, ratio = count / sum(count))

p <- ggplot(num_cg_ratio, aes(area, num_cg)) + stat_bin2d(aes(fill = ratio))
p + mtm + 
    scale_fill_gradientn(colours = rev(rainbow(10, end = 4 / 6)), breaks = seq.int(0, 1, 0.1)) +
    ggtitle("北京二手房各区总价") + xlab("") + ylab("总价（百万元）") + labs(fill = "占比")
```
...
p <- ggplot(houses, aes(area, price))
p + mtm + geom_boxplot() + bmean +
    coord_cartesian(ylim = c(0, 100000)) +
    ggtitle("北京二手房各区单价") + xlab("") + ylab("总价（元）")
...
p <- ggplot(houses, aes(area, meters))
p + mtm + geom_boxplot() + bmean +
    coord_cartesian(ylim = c(0, 300)) +
    ggtitle("北京二手房各区面积") + xlab("") + ylab("面积（平米）")
...
meters_cg_count = ddply(houses, .(area, meters_cg), summarise, count = length(meters_cg))
meters_cg_ratio = ddply(meters_cg_count, .(area), transform, ratio = count / sum(count))

p <- ggplot(meters_cg_ratio, aes(area, meters_cg)) + stat_bin2d(aes(fill = ratio))
p + mtm + 
    scale_fill_gradientn(colours = rev(rainbow(10, end = 4 / 6)), breaks = seq.int(0, 1, 0.1)) +
    ggtitle("北京二手房各区面积") + xlab("") + ylab("面积（平米）") + labs(fill = "占比")
...
p <- ggplot(houses, aes(area, year))
p + mtm + geom_boxplot() + bmean +
    coord_cartesian(ylim = c(1960, 2020)) +
    ggtitle("北京二手房各区建筑年代") + xlab("") + ylab("年代")
...
year_cg_count = ddply(houses, .(area, year_cg), summarise, count = length(year_cg))
year_cg_ratio = ddply(year_cg_count, .(area), transform, ratio = count / sum(count))

p <- ggplot(year_cg_ratio, aes(area, year_cg)) + stat_bin2d(aes(fill = ratio))
p + mtm + 
    scale_fill_gradientn(colours = rev(rainbow(10, end = 4 / 6)), breaks = seq.int(0, 1, 0.1)) +
    ggtitle("北京二手房各区建筑年代") + xlab("") + ylab("建筑年代") + labs(fill = "占比")
...
school_houses <- houses[houses$school == "有学区", ]
school_count = ddply(school_houses, .(area), summarise, count = length(area))
p <- ggplot(school_count, aes(reorder(area, count), count))
p + mtm + bar_label_v +
    geom_bar(stat = "identity", width = I(0.618)) + 
    ggtitle("北京二手房各区学区") + xlab("") + ylab("数量")
...
```{r 房子是不是随着时代的发展越来越大了呢, echo=F}

p <- ggplot(houses, aes(year, meters))
p + mtm + geom_point() + 
    stat_sum_single("mean", geom = "line", color = "red") +
    stat_sum_single("median", geom = "line", color = "blue") +
    scale_x_continuous(breaks = seq(1985, 2020, 5)) +
    coord_cartesian(xlim = c(1985, 2015), ylim = c(0, 200)) +
    ggtitle("面积~建筑年代") + xlab("建筑年代") + ylab("面积（平方)")
...
```{r 学区房房价要比非学区贵多少呢, echo=F}

p <- ggplot(houses, aes(area, price, color = school))
p + mtm + stat_sum_single(median, size = 3) + 
    scale_y_continuous(breaks = seq(0, 70000, 10000)) +
     ggtitle("学区VS非学区房价") + xlab("") + ylab("单价（万元）")
...
```{r 那些地方房价最贵, echo=F}
top10price_con <- topn(ddply(houses, .(con), summarise, sumnum = sum(num), 
                     summeters = sum(meters), price = sumnum / summeters),
                     price, 10, dec = T)

p <- ggplot(top10price_con, aes(reorder(con, price), price))
p + mtm + geom_bar(stat = "identity", width = I(0.618)) +
    ggtitle("北京二手房房价top10区域") + xlab("") + ylab("房价（万元）") +
    coord_flip()
...
num_con <- ddply(houses[houses$num_cg == ">10", ], .(con), summarise, count = length(num_cg))
top10num_con <- topn(num_con, count, 10, dec = T)

p <- ggplot(top10num_con, aes(reorder(con, count), count))
p + mtm + bar_label_h +
    geom_bar(stat = "identity", width = I(0.618)) + 
    ggtitle("北京二手房千万豪宅top10区域") + xlab("") + ylab("数量") +
    coord_flip()
```
...
school_con <- ddply(houses[houses$school == "有学区", ], .(con), summarise, count = length(school))
top10school_con <- topn(school_con, count, 10, dec = T)

p <- ggplot(top10school_con, aes(reorder(con, count), count))
p + mtm + bar_label_h +
    geom_bar(stat = "identity", width = I(0.618)) + 
    ggtitle("北京二手房学区房top10区域") + xlab("") + ylab("数量") +
    coord_flip()
